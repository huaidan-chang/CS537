...
static void process_marithmetic_command(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    case EOM:
        ...
        break;
    case DIVIDE_BY_ZERO:
        errstr = "CLIENT_ERROR cannot divide by zero";
        goto error;
        break;
...
static void process_arithmetic_command(conn *c, token_t *tokens, const size_t ntokens, const int operator) {
    ...
    switch(add_delta(c, key, nkey, operator, delta, temp, NULL)) {
    case OK:
        out_string(c, temp);
        break;
    case NON_NUMERIC:
        if(operator == 0 || operator == 1) {
            ...
            out_string(c, "CLIENT_ERROR cannot operate non-numeric value");
        }
        break;
    case EOM:
